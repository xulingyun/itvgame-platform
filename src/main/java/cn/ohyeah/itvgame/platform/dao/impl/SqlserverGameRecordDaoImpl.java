...
public class SqlserverGameRecordDaoImpl implements IGameRecordDao {
	...
	public void save(String tableName, GameRecord record) {
		QueryHelper.update("insert into "+tableName
				...
				record.getPlayDuration(), record.getScores(), 
				DateUtil.convertToSqlDate(record.getTime()), 
	...
	public void saveOrUpdate(String tableName, GameRecord record) {
		Integer tmp = QueryHelper.read(Integer.class, "select top 1 1 from "
				+tableName+" where accountId=? and recordId=?",
				record.getAccountId(), record.getRecordId());
		if (tmp != null) {
