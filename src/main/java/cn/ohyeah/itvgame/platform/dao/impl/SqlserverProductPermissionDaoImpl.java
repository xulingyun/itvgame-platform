...
public class SqlserverProductPermissionDaoImpl implements IProductPermissionDao {
	...
	public void save(ProductPermission pp) {
		QueryHelper.update("insert into [ProductPermission](accountId, productId, daySubscribeLimit, " +
				"monthSubscribeLimit, lastSubscribeTime, daySubscribeAmount,monthSubscribeAmount, " +
				"totalSubscribeAmount, accessPermission, subscribePermission)" +
				"values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
				pp.getAccountId(), pp.getProductId(),
				pp.getDaySubscribeLimit(), pp.getMonthSubscribeLimit(),
				DateUtil.convertToSqlDate(pp.getLastSubscribeTime()), 
				pp.getDaySubscribeAmount(),	pp.getMonthSubscribeAmount(), 
