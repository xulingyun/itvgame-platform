...
public class MysqlAuthorizationDaoImpl implements IAuthorizationDao {
	...
	public void save(Authorization auth) {
		QueryHelper.update("insert into Authorization(accountId, productID, " 
				...
				auth.getAuthorizationStartTime(), auth.getAuthorizationEndTime());
	...
	public void update(Authorization auth) {
		QueryHelper.update("update Authorization set " +
				"authorizationType=?, leftTryNumber=?, leftValidSeconds=?, leftValidCount=?, " +
				"goldCoin=?, gameCoin=?, "+
				"authorizationStartTime=?, authorizationEndTime=? " +
	...
	public void updateCoins(Authorization auth) {
		QueryHelper.update("update Authorization set goldCoin=?, gameCoin=? where accountId=? and productId=?", 
				auth.getGoldCoin(),	auth.getGameCoin(), auth.getAccountId(), auth.getProductId());
	}
}
